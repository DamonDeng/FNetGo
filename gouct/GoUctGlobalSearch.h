...
ApplyAdditivePredictors(std::vector<SgUctMoveInfo>& moves)
{
    ...
    GoUctAdditiveKnowledgeCNN* kn = dynamic_cast<GoUctAdditiveKnowledgeCNN*> (GetAdditiveKnowledge());
    
    if (kn == 0){
        SgDebug() << "Failed to dynamic case GoUctAdditiveKnowledgeCNN. \n";
        return;
    }
    kn->ProcessPosition(moves, Board());
...
void GoUctGlobalSearchState<POLICY>::GetPrioProbability(SgArray<SgUctValue, SG_MAX_MOVE_VALUE>& array, SgUctValue& value){
    ...
    kn->GetPrioProbability(array, value, historyData);
      ...
      m_globalSearchLiveGfx(GOUCT_LIVEGFX_NONE)
{
    ...
    if (LockFree())
    {
        ...
        // Setting the thread number to 1 for debug.    
        nuThreads = 1;
                            ...
                            unsigned int threadId, const SgUctSearch& search)
{
    ...
    GoAdditiveKnowledge* knowledge = 
    	m_knowledgeFactory.CreateByType(state->Board(), KNOWLEDGE_CNN);
