                            ...
                            const SgUctNode& child) const
{
    ...
    // SgUctValue prioProbability = child.m_prioProbability / 10;
...
SgUctValue SgUctSearch::GetValueEstimateRave(const SgUctNode& child) const
{
    ...
    if (child.HasMean())
    {
        // SgDebug() << "child has mean. \n";
        uctStats.Initialize(child.Mean(), child.MoveCount());
    }
    SgUctStatistics raveStats;
    if (child.HasRaveValue())
    {
        // SgDebug() << "child has rave value. \n";
        raveStats.Initialize(child.RaveValue(), child.RaveCount());
    }
    int virtualLossCount = child.VirtualLossCount();
    if (virtualLossCount > 0)
    {
        // SgDebug() << "virtual loss count > 0. \n";
        uctStats.Add(InverseEstimate(0), SgUctValue(virtualLossCount));
        raveStats.Add(0, SgUctValue(virtualLossCount));
    }
    ...
    if (uctStats.IsDefined())
    {
        ...
        if (hasRave)
        {
            ...
            // double in_end_time;
            // in_end_time = timer.GetTime();
            // SgDebug() << "# in rave: " << (in_end_time - in_start_time) << ". \n";
        }
        else
        {
            // SgDebug() << "not have rave. \n";
...
bool SgUctSearch::PlayInTree(SgUctThreadState& state, bool& isTerminal)
{
    ...
    while (true)
    {
        ...
        if (! current->HasChildren())
        {
            ...
            if (current->MoveCount() >= m_expandThreshold)
            {
                ...
                SgUctNode* noneConstCurrent = const_cast<SgUctNode*> (current);

                if (state.m_needPrioProbability){
                    noneConstCurrent->m_childPrioProbabilityComputed = true;
                }
        ...
        // double in_middle_time;
        // in_middle_time = timer.GetTime();
        ...
        // SgDebug() << "# middle to start in while loop: " << in_middle_time - in_start_time << ". \n";
        // SgDebug() << "# end to middle in while loop: " << in_end_time - in_middle_time << ". \n";
    ...
    // SgDebug() << "# while loop number: " << f << ".   time in playinTree: " << end_time - start_time << ". \n";
                               ...
                               SgUctEarlyAbortParam* earlyAbort)
{
    ...
    std::vector<SgUctValue> childPosCount(SG_MAX_MOVE_VALUE);
    std::vector<SgUctValue> childPrioPosCount(SG_MAX_MOVE_VALUE);

    std::vector<SgUctValue> childPrioProbability(SG_MAX_MOVE_VALUE);
    ...
    for (SgUctChildIterator it(m_tree, m_tree.Root()); it; ++it){
        if ((*it).Move() == SG_PASS){
            ...
            childProbability[(*it).Move()] = (*it).m_predictorValue;
            childPrioProbability[(*it).Move()] = (*it).m_prioProbability;
            ...
            childPosCount[(*it).Move()] = (*it).PosCount();
            childPrioPosCount[(*it).Move()] = (*it).m_prioPosCount;
                                          ...
                                          const SgUctNode& node)
{
    ...
    SgUctValue prioPosCount = node.m_prioPosCount;
    SgUctValue moveCount = node.MoveCount();
    ...
    for (SgUctChildIterator it(m_tree, node); it; ++it)
    {
        ...
        if (! child.IsProvenWin()) // Avoid losing moves
        {
            ...
            // SgDebug() << "# in select child: " << in_end_time - in_start_time << ". \n";
    ...
    // SgDebug() << "# f: " << f << "in select child: " << in_end_time - in_start_time << ". \n";
