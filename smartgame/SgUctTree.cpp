...
void SgUctTree::ApplyPrioProbabilityToChildren(std::size_t allocatorId, const SgUctNode& node, SgArray<SgUctValue, SG_MAX_MOVE_VALUE>& array, SgUctValue value, int threadID){
        ...
        // the following cord are hacking implemetations, 
        // which copy all the const var from GoUctAdditiveKnowledge

        float predictorTotal = 0.0;
        float minValue = 0.0001f;
        float predictorMax = minValue;

        for (SgUctChildIterator it(*this, node); it; ++it){
            
            const SgUctNode& childNode = *it;
            // SgUctNode& noneConstChild = const_cast<SgUctNode&> (child);
            // noneConstChild.m_predictorValue = 0.83838;

            float originalValue = 0.0f;

            if (childNode.Move() == SG_PASS){
                originalValue = array[SG_MAX_MOVE_VALUE-1];
            } else {
                if (childNode.Move() >= SG_MAX_MOVE_VALUE){
                    SgDebug() << "incorrect move value: " << childNode.Move() << " !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n";
                } else {
                    originalValue = array[childNode.Move()];
                }
            }

            predictorTotal += std::max(minValue, originalValue);
            if (originalValue > predictorMax) {
                predictorMax = originalValue;
            }
                

        }

        float knowledgeScale = 0.03f;

        
        
        const float scale = knowledgeScale * sqrtf(predictorTotal);
        ...
        //     // SgDebug() << "in the loop." << std::endl;
            ...
            if (child->Move() == SG_PASS){
                ...
                if (child->Move() >= SG_MAX_MOVE_VALUE){
                    ...
                    originalValue = array[child->Move()];
                    
                }
            }

            const float v = std::max(minValue, originalValue);

            child->m_predictorValue = scale * (1 / sqrtf(v));

            child->m_prioProbability = originalValue;
