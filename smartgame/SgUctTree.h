...
public:
    ...
    // added by Damon, the flag of child prioProbability.
    bool m_childPrioProbabilityComputed;

    /* Value of additive predictor */
    // moving it to public attribute for visiability, it is a hacking implementation, 
    //@todo, add normal access function
    volatile float m_predictorValue;

    volatile float m_prioProbability;

    volatile float m_prioPosCount;
...
inline SgUctNode::SgUctNode(const SgUctMoveInfo& info)
    : m_childPrioProbabilityComputed(false),
...
inline void SgUctNode::CopyDataFrom(const SgUctNode& node)
{
    ...
    m_childPrioProbabilityComputed = node.m_childPrioProbabilityComputed;
    m_prioProbability = node.m_prioProbability;
    m_prioPosCount = node.m_prioPosCount;
}
...
{
    ++m_posCount;
    if (m_childPrioProbabilityComputed){
        ++m_prioPosCount;
    }
}
...
inline void SgUctNode::DecPosCount()
{
    ...
    if (m_childPrioProbabilityComputed){
        SgUctValue posCount = m_prioPosCount;
        if (posCount > 0)
        {
            m_prioPosCount = posCount - 1;
        }
    }
}
